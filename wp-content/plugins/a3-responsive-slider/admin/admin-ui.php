...
class A3_Responsive_Slider_Admin_UI
{
	...
	public $version_transient = 'a3rev_rslider_update_info';

	public $plugin_option_key = 'a3rev_rslider_plugin';

	public $support_url = 'https://a3rev.com/forums/forum/wordpress-plugins/a3-responsive-slider/';
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'a3_responsive_slider' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'a3_responsive_slider' ), $this->support_url );
			} else {
				...
				if ( FALSE !== stristr( $version_transient, '||' )
					...
					&& version_compare( get_option('a3rev_rslider_version') , $version_info[0], '<' ) ) {
						...
						$version_message = sprintf( __( 'There is a new version <span class="a3rev-ui-new-plugin-version">%s</span> available, <a href="%s" target="_blank">update now</a> or download direct from <a href="%s" target="_blank">My Account</a> on a3rev.com', 'a3_responsive_slider' ),
							$version_info[0],
